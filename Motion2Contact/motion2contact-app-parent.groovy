...
def mainPage() {
    dynamicPage(name: "mainPage") {
        section("") {
            paragraph "Sync motion sensor events to a virtual contact sensor. The virtual contact sensor can then be used instead of the motion sensor in HSM as one of the intrusion delay devices as only contact sensors are available to be selected."
            paragraph "<br><i>motion active -> contact open</i>"
            paragraph "<i>motion inactive -> contact closed</i>"
        }
        section() {
            app(name: "anyOpenApp", appName: "Motion2Contact Child", namespace: "dcoghlan", title: "Create a new '${app.label}' child", multiple: true)
        }
        displayFooter()
	}
}
